...
def set_channel_dict(channel_dict=None):
    if type(channel_dict) == None:
        channel_dict = {
            'ch-0' : 'donor', 
            'ch-1' : 'acceptor',
            'ch-2' : 'directAcceptor',
...
#fix this so channel dicts can be fixed
def get_channel_name(channel_index, channel_dict=None, experiment_type=None):
    """
    ...
    """
    channel_dict = set_channel_dict(channel_dict)
    ...
    if experiment_type == 'osmotic_perturbation':
        ...
        try:
            ...
            key = channel_index if isinstance(channel_index, str) else f'ch-{channel_index}'
