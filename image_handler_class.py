...
class ImageAlignmentAndTifConversion:
    ...
    def __init__(self, 
        ...
        if isinstance(stack_method, str):
            self.stack_method = image_functions.stack_method(stack_method)
        if stack_method is None:
            print('Default Stack method: rigid_body')
            self.stack_method = image_functions.stack_method('rigid_body')
        ...
        try:
            if 't' in images.dims:
                ...
                for timepoint in range(images.sizes['t']):
                    ...
                    aligned = image_functions.image_alignment(cur_image_t, stat_image,  self.cam2_alignment_register, excluded_channel)
